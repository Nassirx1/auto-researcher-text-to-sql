...
def ecommerce_templates() -> list[Template]:
    return [
        ...
        lambda r: ("hard", "Which regions have the highest refund-adjusted category revenue rank?", "WITH category_region AS (SELECT rg.region_name, cat.category_name, SUM(oi.quantity * oi.unit_price) AS gross_revenue, COALESCE(SUM(rf.amount), 0) AS refunds FROM regions rg JOIN customers c ON rg.region_id = c.region_id JOIN orders o ON c.customer_id = o.customer_id JOIN order_items oi ON o.order_id = oi.order_id JOIN products p ON oi.product_id = p.product_id JOIN categories cat ON p.category_id = cat.category_id LEFT JOIN refunds rf ON o.order_id = rf.order_id WHERE o.order_status = 'completed' GROUP BY rg.region_name, cat.category_name), ranked AS (SELECT region_name, category_name, gross_revenue - refunds AS net_revenue, RANK() OVER (PARTITION BY region_name ORDER BY gross_revenue - refunds DESC) AS revenue_rank FROM category_region) SELECT region_name, category_name, ROUND(net_revenue, 2) AS net_revenue FROM ranked WHERE revenue_rank = 1 ORDER BY net_revenue DESC"),
        lambda r: ("hard", "Which campaigns drove repeat purchasers with support tickets after conversion?", "WITH converted AS (SELECT DISTINCT ws.customer_id, mc.campaign_name FROM web_sessions ws JOIN marketing_campaigns mc ON ws.campaign_id = mc.campaign_id WHERE ws.converted = 1), repeat_buyers AS (SELECT customer_id FROM orders WHERE order_status = 'completed' GROUP BY customer_id HAVING COUNT(*) >= 2), ticketed AS (SELECT customer_id, COUNT(*) AS tickets FROM support_tickets GROUP BY customer_id) SELECT c.campaign_name, COUNT(DISTINCT c.customer_id) AS repeat_customers_with_tickets, SUM(t.tickets) AS total_tickets FROM converted c JOIN repeat_buyers rb ON c.customer_id = rb.customer_id JOIN ticketed t ON c.customer_id = t.customer_id GROUP BY c.campaign_name ORDER BY repeat_customers_with_tickets DESC LIMIT 20"),
    ]
...
def saas_templates() -> list[Template]:
    return [
        ...
        lambda r: ("hard", "Which plans have declining usage but increasing unpaid invoice exposure?", "WITH monthly_usage AS (SELECT s.plan_id, substr(u.usage_date, 1, 7) AS month, SUM(u.events) AS events FROM product_usage u JOIN subscriptions s ON u.account_id = s.account_id GROUP BY s.plan_id, month), usage_lag AS (SELECT plan_id, month, events, LAG(events) OVER (PARTITION BY plan_id ORDER BY month) AS previous_events FROM monthly_usage), unpaid AS (SELECT s.plan_id, SUM(i.amount) AS unpaid_amount FROM invoices i JOIN subscriptions s ON i.account_id = s.account_id WHERE i.status = 'unpaid' GROUP BY s.plan_id) SELECT p.plan_name, COUNT(*) AS declining_months, ROUND(COALESCE(u.unpaid_amount, 0), 2) AS unpaid_amount FROM usage_lag ul JOIN plans p ON ul.plan_id = p.plan_id LEFT JOIN unpaid u ON ul.plan_id = u.plan_id WHERE ul.previous_events IS NOT NULL AND ul.events < ul.previous_events GROUP BY p.plan_name, u.unpaid_amount ORDER BY unpaid_amount DESC, declining_months DESC"),
        lambda r: ("hard", "Which churned accounts had both high severity support and low pre-churn usage?", "WITH severe_support AS (SELECT account_id, COUNT(*) AS severe_tickets FROM support_tickets WHERE severity IN ('high', 'critical') GROUP BY account_id), usage_before_churn AS (SELECT ce.account_id, SUM(pu.events) AS pre_churn_events FROM churn_events ce JOIN product_usage pu ON ce.account_id = pu.account_id WHERE pu.usage_date <= ce.churn_date GROUP BY ce.account_id) SELECT a.account_name, ce.reason, COALESCE(ss.severe_tickets, 0) AS severe_tickets, COALESCE(ubc.pre_churn_events, 0) AS pre_churn_events FROM churn_events ce JOIN accounts a ON ce.account_id = a.account_id LEFT JOIN severe_support ss ON ce.account_id = ss.account_id LEFT JOIN usage_before_churn ubc ON ce.account_id = ubc.account_id WHERE COALESCE(ss.severe_tickets, 0) > 0 AND COALESCE(ubc.pre_churn_events, 0) < 1000 ORDER BY severe_tickets DESC, pre_churn_events ASC"),
    ]
...
def banking_templates() -> list[Template]:
    return [
        ...
        lambda r: ("hard", "Which branches combine risky loan exposure with suspicious transaction activity?", "WITH branch_fraud AS (SELECT b.branch_id, b.branch_name, COUNT(*) AS transactions, SUM(t.suspicious) AS suspicious_transactions FROM branches b JOIN accounts a ON b.branch_id = a.branch_id JOIN transactions t ON a.account_id = t.account_id GROUP BY b.branch_id, b.branch_name), branch_loans AS (SELECT branch_id, SUM(CASE WHEN risk_grade IN ('C', 'D') THEN principal ELSE 0 END) AS risky_principal FROM loans GROUP BY branch_id) SELECT bf.branch_name, ROUND(bl.risky_principal, 2) AS risky_principal, ROUND(bf.suspicious_transactions * 1.0 / bf.transactions, 4) AS suspicious_rate FROM branch_fraud bf JOIN branch_loans bl ON bf.branch_id = bl.branch_id WHERE bl.risky_principal > 0 ORDER BY risky_principal DESC, suspicious_rate DESC"),
        lambda r: ("hard", "Which customers have high balances, active cards, and unresolved fraud alerts?", "WITH alert_counts AS (SELECT a.customer_id, COUNT(*) AS unresolved_alerts FROM fraud_alerts fa JOIN transactions t ON fa.transaction_id = t.transaction_id JOIN accounts a ON t.account_id = a.account_id WHERE fa.resolved = 0 GROUP BY a.customer_id), active_cards AS (SELECT a.customer_id, COUNT(*) AS active_cards FROM cards c JOIN accounts a ON c.account_id = a.account_id WHERE c.active = 1 GROUP BY a.customer_id), balances AS (SELECT customer_id, SUM(balance) AS total_balance FROM accounts GROUP BY customer_id) SELECT c.customer_name, ROUND(b.total_balance, 2) AS total_balance, ac.active_cards, al.unresolved_alerts FROM customers c JOIN balances b ON c.customer_id = b.customer_id JOIN active_cards ac ON c.customer_id = ac.customer_id JOIN alert_counts al ON c.customer_id = al.customer_id WHERE b.total_balance > 25000 ORDER BY al.unresolved_alerts DESC, total_balance DESC LIMIT 25"),
    ]
...
def main() -> None:
    ...
    parser.add_argument("--stress-eval", action="store_true", help="Make eval hard-only to better measure relational reasoning.")
    ...
    eval_rows = []
    eval_difficulties = {"hard"} if args.stress_eval else None
    for row in generate_examples(eval_count * 10, 5001, args.seed + 999, database_dir, args.include_sample_rows):
        if eval_difficulties and row["difficulty"] not in eval_difficulties:
            continue
